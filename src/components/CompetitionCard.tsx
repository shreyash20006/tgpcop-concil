...
export const CompetitionCard: React.FC<CompetitionCardProps> = ({ competition }) => {
  ...
  useEffect(() => {
    const calculateTimeLeft = () => {
      const difference = +new Date(`${competition.deadline}T23:59:59`) - +new Date();
      let newTimeLeft = {
        days: 0,
        hours: 0,
        minutes: 0,
      ...
      <div className="p-6 flex flex-col h-full justify-between">
          ...
          <div className="space-y-3 mb-6 bg-navy-dark/5 p-4 rounded-lg">
            <div className="flex items-start space-x-2 text-xs">
              ...
              <div>
                ...
                <span className="text-navy-dark/80">{competition.prizeInfo}</span>
              </div>
            </div>
            <div className="flex items-start space-x-2 text-xs">
              ...
              <div>
                ...
                <span className="text-navy-dark/80">{competition.participationInfo}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          {timeLeft.isExpired ? (
            <button
              disabled
              className="w-full py-3 bg-navy-dark/10 text-navy-dark/40 font-display font-semibold rounded-lg text-sm cursor-not-allowed"
            >
              Registrations Closed
            </button>
          ) : (
            <a
              href={competition.googleFormUrl}
